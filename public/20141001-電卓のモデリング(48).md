...
tags:
  ...
  - oop
private: true
updated_at: '2014-10-09T09:36:58+09:00'
id: 833d4a81695db93404db
...
## 状態を認識する
あるイベントに対して考えられるアクションが複数の場合がありませんか？
もしそのような場合には，状態 (state) を分けます。

たとえば， GoodBye アプリのことを思い出してください。同じ「ボタン押下」というイベントに対して，Hello, World と表示したり Good Bye と表示したり，反応が変化しましたね。このようなときには状態を分けます。よくよく考えてみると次のようになっています。

* 現在 Hello, World と表示している時には Good Bye と表示する。
* 現在 Good Bye と表示している時には Hello, World と表示する。

そこで，前者を Hello 状態，後者を GoodBye 状態と名前をつけます。一般に「◯◯している時には」という表現があった時には，「◯◯」状態とか「◯◯中」状態などといった名前をつけるとわかりやすくなります。

もし，このヒントに着想を得て，状態機械モデリングが続けられるならば，自力で挑戦してみてください。このあとで何をしたらいいか10分以上考えてもわからなかったら，[次のページに進んでヒントを読んでください。](http://qiita.com/zacky1972/private/f55ba97d5de5576d39dc)
